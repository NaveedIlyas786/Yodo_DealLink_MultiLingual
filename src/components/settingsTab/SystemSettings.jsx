...
const SystemSettings = () => {
  ...
  return (
    <div className='flex-1 lg:flex-[0.60] flex flex-col gap-[20px] bg-gray-50 rounded'>
      <div>
        <h1 className='text-xl kumbh_sans_SubTitels mb-1'>
          {t('Show Notifications')}
        </h1>
        <p className='kumbh_sans_normal_bold text-gray-600 mb-4'>
          {t(
            'Allow to receive push notifications for user activities and logs count.'
          )}
        </p>
        <div className='flex items-center gap-4'>
          <span className='Helviticaneue_16 text-gray-700'>{t('OFF')}</span>
          <label className='switch'>
            <input type='checkbox' />
            <span className='slider round'></span>
          </label>
          <span className='Helviticaneue_16 text-gray-700'>{t('ON')}</span>
        </div>
      </div>
